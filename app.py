...
def _fetch_one_stooq(t, start, end):
    # Stooq requiere pandas-datareader; tickers US suelen funcionar tal cual (AAPL, MSFT, SPY)
    try:
...
def fetch_prices_resilient(tickers, start=None, end=None, interval="1d"):
    """
    1) Bulk yfinance
    2) Individual yfinance (download/history)
    ...
    # Falta alguno -> intenta individual y luego stooq
    for t in uniq:
    ...
    # Filtra columnas vacías por seguridad
...
def last_prices_resilient(tickers):
    uniq = _clean_tickers(tickers)
    if not uniq: return {}, []
    ok, failed = {}, []
    # bulk 5d
    try:
        ...
        close = _flatten_close(df, uniq).ffill().dropna(how="all")
    ...
    for t in uniq:
        ...
        try:
            ...
            if not h.empty and "Close" in h:
                ok[t] = float(h["Close"].dropna().iloc[-1]); got = True
        except Exception:
            pass
        if not got:
            try:
                ...
                if d is not None and not d.empty and "Close" in d:
                    ok[t] = float(d.sort_index()["Close"].dropna().iloc[-1]); got = True
            except Exception:
                pass
        if not got:
            failed.append(t)
...
def positions_from_tx(tx:pd.DataFrame):
    ...
    if not uniq:
        return pd.DataFrame(columns=["Ticker","Shares","AvgCost","Invested","MarketPrice","MarketValue","UnrealizedPL"])
    last_map, failed_lp = last_prices_resilient(uniq)
    pos=[]
...
if page=="Mi Portafolio":
    ...
    # Bajamos cartera + benchmark (con proxy si falla ^GSPC)
    tickers_plus = tickers + [benchmark]
    prices, failed = fetch_prices_resilient(tickers_plus, start=start_date)

    bench_series = pd.Series(dtype=float)
    if prices.empty or benchmark not in prices.columns:
        # Usa SPY como proxy si el benchmark no bajó
        if "SPY" not in tickers:
            prices_proxy, _ = fetch_prices_resilient(tickers + ["SPY"], start=start_date)
            if not prices_proxy.empty and "SPY" in prices_proxy.columns:
                bench_series = prices_proxy["SPY"].pct_change().dropna()
                prices = prices_proxy.drop(columns=["SPY"], errors="ignore")
    else:
    ...
    if prices.empty:
        ...
        st.warning(msg + " Verifica símbolos o intenta más tarde.")
        st.stop()
...
# ================== OPTIMIZAR ==================
    ...
    tickers = pos_df["Ticker"].tolist()
    prices, failed = fetch_prices_resilient(tickers, start=start_date)
    if prices.empty:
    ...
    for t in tickers:
        ...
        if delta!=0:
            ...
            est=abs(delta)*px + get_setting(settings_df,"FeePerTrade",0.0,float) + (abs(delta)*px)*(get_setting(settings_df,"SlippageBps",0,float)/10000)
...
# ================== EVALUAR CANDIDATO ==================
    ...
    prices, failed = fetch_prices_resilient(all_tickers, start=start_date)
    if prices.empty:
    ...
    last_map,_ = last_prices_resilient([tkr]); last=last_map.get(tkr,np.nan)
...
# ================== HERRAMIENTAS ==================
    ...
    with tabs[0]:
        ...
        if pos_df.empty: st.info("Sin posiciones.")
            ...
            tickers=pos_df["Ticker"].tolist()
            prices, _ = fetch_prices_resilient(tickers+[benchmark], start=start_date)
            if prices.empty or benchmark not in prices.columns:
                # proxy SPY
                prices, _ = fetch_prices_resilient(tickers+["SPY"], start=start_date)
    ...
    with tabs[2]:
        ...
        if pos_df.empty: st.info("Sin posiciones.")
            ...
            tickers=pos_df["Ticker"].tolist()
            prices,_=fetch_prices_resilient(tickers, start=start_date)
