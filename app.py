...
def fetch_prices(tickers, start=None, end=None, interval="1d"):
    """Descarga Close para lista de tickers. Devuelve DataFrame (cols=tickers). Maneja vacíos."""
    if not tickers:
        return pd.DataFrame()
    # yfinance acepta string o lista; normalizamos lista única
    unique = sorted({t for t in tickers if isinstance(t, str) and t.strip()})
    if not unique:
        return pd.DataFrame()
    if start is None:
    ...
    try:
        data = yf.download(unique, start=start, end=end, interval=interval, auto_adjust=True, progress=False)
        if data is None or len(data) == 0:
            return pd.DataFrame()
        # Cuando es 1 ticker, yfinance devuelve Series para "Close"
        close = data["Close"] if "Close" in data else data
        if isinstance(close, pd.Series):
            close = close.to_frame(name=unique[0])
        # Hay casos con columnas MultiIndex; intentamos aplanar
        if isinstance(close.columns, pd.MultiIndex):
            close.columns = [c[-1] if isinstance(c, tuple) else c for c in close.columns]
        close = close.loc[:, [c for c in close.columns if c in unique]]
...
def last_prices(tickers):
    """Último precio por ticker como dict. Maneja vacíos y Series/DF."""
    if not tickers:
        return {}
    unique = sorted({t for t in tickers if isinstance(t, str) and t.strip()})
    if not unique:
        return {}
    try:
        info = yf.download(unique, period="5d", interval="1d", auto_adjust=True, progress=False)
        if info is None or len(info) == 0:
            return {}
        close = info["Close"] if "Close" in info else info
        if isinstance(close, pd.Series):
            close = close.to_frame(name=unique[0])
        ...
        if close.empty:
            return {}
        last_row = close.tail(1)  # evita iloc[-1] sobre DF vacío
        # Convertimos a dict (col -> valor)
        result = {col: float(last_row.iloc[0][col]) for col in last_row.columns if pd.notna(last_row.iloc[0][col])}
        return result
    except Exception:
        return {}
...
def annualize_return(daily_ret: pd.Series, freq=252):
    if daily_ret.empty:
        return np.nan
    m = (1 + daily_ret).prod() ** (freq / max(len(daily_ret), 1)) - 1
    return float(m)
...
def tidy_transactions(tx: pd.DataFrame) -> pd.DataFrame:
    ...
    def signed(row):
        ...
        if side in ("buy","compra","1"):
            return abs(q)
        return q  # respeta signo si no hay Side
...
def positions_from_tx(tx: pd.DataFrame):
    ...
    px_last = last_prices(unique_tickers)
    pos = []
    for tkr, grp in df.groupby("Ticker"):
        shares = float(grp["SignedShares"].sum())
        if abs(shares) < 1e-12:
            continue
        # avg cost solo con compras
        buy_mask = grp["SignedShares"] > 0
        if buy_mask.any():
            total_shares = float(grp.loc[buy_mask,"SignedShares"].sum())
...
if SHOW_DEBUG:
    st.caption(f"Secrets: SHEET_ID set={bool(SHEET_ID)}, SA={bool(GCP_SA)}, Benchmark={benchmark}")
...
if page == "Mi Portafolio":
    ...
    if prices.empty:
        st.warning("No se pudieron obtener precios históricos. Verifica tickers y/o intenta más tarde.")
        st.stop()
    ...
    if benchmark in prices.columns:
        bench_series = prices[[benchmark]].dropna().pct_change().dropna()[benchmark]
    port_ret, asset_rets = constant_weight_portfolio_returns(prices.drop(columns=[c for c in [benchmark] if c in prices.columns]), w)

    # KPIs
    col1,col2,col3,col4,col5,col6 = st.columns(6)
    col1.metric("Rend. anualizado", f"{(annualize_return(port_ret) or 0)*100:,.2f}%")
    col2.metric("Vol. anualizada", f"{(annualize_vol(port_ret) or 0)*100:,.2f}%")
    col3.metric("Sharpe", f"{(sharpe(port_ret, rf) or 0):.2f}")
    col4.metric("Sortino", f"{(sortino(port_ret, rf) or 0):.2f}")
    cum = (1+port_ret).cumprod() if not port_ret.empty else pd.Series(dtype=float)
    mdd = max_drawdown(cum) if not cum.empty else np.nan
    col5.metric("Max Drawdown", f"{(mdd or 0)*100:,.2f}%")
    col6.metric("Calmar", f"{(calmar(port_ret) or 0):.2f}")

    # vs Benchmark
    if not bench_series.empty and not port_ret.empty:
    ...
    if not port_ret.empty:
        perf_df["Portafolio"] = (1+port_ret).cumprod()
    ...
    st.dataframe(
        pos_df[show_cols].style.format({"AvgCost":"$,.2f","MarketPrice":"$,.2f","MarketValue":"$,.2f","UnrealizedPL":"$,.2f"}),
    ...
    if not port_ret.empty and calmar(port_ret) is not np.nan and calmar(port_ret) < 0.3:
...
# ============ OPTIMIZE & REBALANCE ============
    ...
    if prices.empty:
        st.warning("No se pudieron obtener precios de los activos actuales.")
        st.stop()

    w_cur = weights_from_positions(pos_df)
    port_ret_cur, asset_rets = constant_weight_portfolio_returns(prices, w_cur)
    if asset_rets.empty:
        st.warning("No hay suficientes retornos para optimizar.")
        st.stop()
    ...
    w_opt_arr = markowitz_max_sharpe(mu_ann.values, cov.values, rf=rf, bounds=bounds)
    if w_opt_arr.size == 0:
        st.warning("No se pudo calcular una solución de optimización.")
        st.stop()

    w_opt = pd.Series(w_opt_arr, index=tickers).clip(lower=w_min, upper=w_max)
    if w_opt.sum() <= 0:
        st.warning("Pesos propuestos inválidos.")
        st.stop()
    w_opt /= w_opt.sum()
    ...
    for t in tickers:
        ...
        if px <= 0:
            continue
        tgt_value = float(portfolio_value * w_opt.get(t, 0.0))
...
# ============ EVALUAR CANDIDATO ============
    ...
    if prices.empty:
        st.warning("No se pudieron descargar precios para el candidato.")
        st.stop()
    ...
    # Métricas Δ
    core_cols = [c for c in prices.columns if c != benchmark]
    port_cur, assets_cur = constant_weight_portfolio_returns(prices[core_cols], w_cur.reindex(core_cols).fillna(0))
    port_new, assets_new = constant_weight_portfolio_returns(prices[core_cols], w_new.reindex(core_cols).fillna(0))
    rf_local = rf

    cols = st.columns(4)
    cols[0].metric("Sharpe actual", f"{(sharpe(port_cur, rf_local) or 0):.2f}")
    sh_new = sharpe(port_new, rf_local)
    sh_old = sharpe(port_cur, rf_local)
    ...
    with sub[0]:
        ...
        if pos_df.empty:
            ...
            if prices.empty or benchmark not in prices.columns:
                ...
                core_cols = [c for c in prices.columns if c != benchmark]
                w = weights_from_positions(pos_df)
                port_ret, _ = constant_weight_portfolio_returns(prices[core_cols], w)
